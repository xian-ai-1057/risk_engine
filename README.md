...
| 缺值傳播 | 任一代碼缺失 → 公式 `None` → 規則 `missing`。條件樹採三值短路：**AND** 任一葉 `false` → 整體 `not_triggered`；**OR** 任一葉 `true` → 整體 `triggered`；其他情況若有葉缺值才整棵 `missing`。 |
| AND/OR 分割 | 先以 `\s+OR\s+` 分割（OR 優先），再以 `\s+AND\s+` 分割（AND 次之）；單條件以右起最後一個比較運算子切分公式與門檻。 |
| 全形符號 | `＞` / `＜` / `＝` 自動正規化為半形。 |
| 單位推斷 | 由 config 的 `result_unit` 優先；否則由公式所有代碼的 `單位` 推斷（單位一致且不含除法時採用，**同單位的除法視為無量綱**；若公式末端含外層 `*<常數>` 將比率放大為原單位（如 `...*100` 表「百分點」），仍沿用 operand 單位）。 |
...
result = pipe.run()
# result["narrative_prompt"]  → 合併後的敘事 Prompt（filter-driven）
# result["risk_prompt"]       → 合併後的風險 Prompt（rules-driven）
# result["grouped_report"]    → 依 filter 分群的報表（同 group_sample 格式）
