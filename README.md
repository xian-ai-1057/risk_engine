...
CLI 與 Pipeline 兩條路徑共用底層模組，呼叫關係如下：

```
                ┌───────────────────────────────┐
   CLI 入口     │ risk_checker.main             │
                │  └─ _run                      │
                └──────────────┬────────────────┘
                               │
                ┌──────────────┴────────────────┐
                ▼                               ▼
   ┌───────────────────────────┐   ┌───────────────────────────┐
   │ loader.load_report        │   │ loader.load_config         │
   │  ├─ _load_report_csv      │   │  └─ json.load + 篩選產業    │
   │  └─ _load_report_json     │   └─────────────┬──────────────┘
   └─────────────┬─────────────┘                 │
                 └────────────┬───────────────────┘
                              ▼
              ...
              │          │   ├─ _substitute_codes
              │          │   └─ _safe_eval (_tokenize → _Parser)
              │          ├─ formula.classify_formula
              │          ├─ formula.extract_operands
              │          ├─ checker.check_rule  ──┐
              │          └─ _enrich_condition_…  │
              └──────────────┬───────────────────┘
                             │           ┌─ _check_absolute
                             │  _HANDLERS┼─ _check_period_change
                             │           └─ _check_compound
                             ...
                             ▼
              ┌──────────────────────────────────┐
              │ post_rules.apply_post_rules      │  (預留，目前 pass-through)
              └──────────────┬───────────────────┘
                             ▼
              ┌──────────────────────────────────┐
              │ report.to_llm_format  (--compact)│
              │ report.to_prompt_view (Pipeline) │
              │ narrative.build_narrative (--narrative)
              └──────────────────────────────────┘

                ┌───────────────────────────────┐
   Pipeline 入口│ ReportPipeline.run             │
                │  ├─ filter_and_group           │
                │  │    └─ narrative.extract_section_codes
                │  ├─ build_narrative_prompt     │
                │  │    └─ combine_prompt.render_narrative_prompt
                │  └─ build_risk_prompt          │
                │       ├─ report.generate_report (同上分支)
                │       ├─ post_rules.apply_post_rules
                │       └─ combine_prompt.render_risk_prompt
                │             └─ render_prompt
                │                  ├─ report.to_prompt_view
...
```
  ┌────────────┐  loader        ┌────────────┐
  │ CSV / JSON │ ─────────────▶ │  Report    │   {code: ReportRow}
  │  財報原檔   │   load_report  │  (types)   │   ReportRow = {FA_CANME, 單位,
  └────────────┘                └─────┬──────┘                  Current, Period_2, Period_3}
                                      │
                                      │  ┌─────────────────────────────────┐
                                      ├─▶│  Pipeline.filter_and_group       │
                                      │  │  + narrative.extract_section_…   │
                                      │  └────────────────┬────────────────┘
                                      │                   ▼
                                      │      GroupedReport {section: {code: ReportRow}}
                                      │                   │
                                      │                   ▼
                                      │      combine_prompt.render_narrative_prompt
                                      │                   │
                                      │                   ▼ (json.dumps → 替換 {{JSON_DATA}})
       ...
       ┌──────────────────────┼─────────────────────────────┐
       ▼                      ▼                             ▼
  formula.evaluate_formula  formula.extract_operands   formula.classify_formula
  (current/prev: float|None)  (Operand list)            (value_kind, value_label)
       │
       ▼
  checker.check_rule
  ├─ absolute / period_change_*  → 直接以 current/prev 比對
  └─ compound → evaluate_node(condition_tree, report) 遞迴
                   → ConditionDetail list（formula, value, operator, threshold, result）
       │
       ▼
  TagResult {tag_id, status, threshold, description, condition_details?}
       │  ↑ _enrich_condition_details 補 subject + display
       ▼
  IndicatorEntry {indicator_name, indicator_code,
                  current_value/display, previous_value/display,
                  value_kind, value_label, operands, taggings}
       │
       ▼
  FullReport {customer_id, report_date, industry, summary, sections}
       │
       ├─ post_rules.apply_post_rules（pass-through，預留 meta-rule）
       │
       ├─ to_llm_format  → {section: [{n, cur, prev?, tags:[{s, th?, d?}]}]}（CLI --compact）
       │
       └─ to_prompt_view → 剝除代碼/原始數值的視圖
                │
                ▼
        combine_prompt.render_prompt
                ├─ {{risk_results_1..5}} ← json.dumps(prompt_view[section])
                └─ {{narrative_1..5}}    ← format_narrative_text(NarrativeItem list)
                                                ▲
                                                │
                                  narrative.build_narrative(report, rules, tag_table)
                                                │
                                                ▼
                          NarrativeSections {section: [NarrativeItem]}
                          NarrativeItem = {name, unit, current, period_2, period_3}
                │
                ▼
            risk_prompt: str
```
...
```
  float (raw)
     ...
     ▼  UNIT_FORMATTERS[unit]
        ├─ "仟元" → convert_thousand_ntd  → "NTD 1,099,433仟元" / "NTD 924,470仟元"
        ├─ "%"   → format_percent        → "12.93%"
        ├─ "天"   → format_days           → "85.44天"
        └─ "倍"   → format_times          → "0.55倍"

  ReportRow (Current / Period_2 / Period_3)
     │  convert_grouped_report(period_dates=[...])
     ▼
  {section: {code: {FA_CANME, 趨勢, "MM/DD/YYYY": "顯示值", ...}}}
                                ▲
                                └─ _calc_trend：>±5% 為 up/down，否則 flat；
                                                組合為「逐期上升 / 大致持平 / 呈先升後降走勢」等字串
```
